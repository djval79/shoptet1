...
import { BusinessProfile, Task, Customer, Order } from '../types';
...
const Tasks: React.FC<TasksProps> = ({ business, tasks, customers = [], orders = [], onUpdateTasks }) => {
    ...
    const handlePlanDay = async () => {
        ...
        try {
            const result = await generateSmartTasks(customers, orders, business);
    ...
    const renderTaskCard = (task: Task) => (
        <div key={task.id} className="bg-slate-800 p-4 rounded-lg border border-slate-700 shadow-sm hover:border-blue-500/50 transition-all group animate-in fade-in zoom-in-95 duration-300">
    ...
    return (
        <div className="max-w-6xl mx-auto animate-in fade-in duration-500 h-[calc(100vh-6rem)] flex flex-col">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex space-x-3">
                    ...
                    <button
                        ...
                        className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white px-6 py-2 rounded-lg font-medium shadow-lg shadow-purple-900/20 flex items-center disabled:opacity-50 transition-all"
                    >
                        {isGenerating ? (
                            <>
            ...
            {/* Add Task Form */}
            {isAdding && (
                <form onSubmit={handleAddTask} className="mb-6 bg-slate-800 p-4 rounded-xl border border-slate-700 animate-in slide-in-from-top-2">
                    <div className="flex gap-2">
                        <input
                            ...
                            className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white focus:border-blue-500 outline-none"
                            autoFocus
                        />
                        <button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-2 rounded-lg font-medium">Add</button>
                    </div>
                </form>
            )}
