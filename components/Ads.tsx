...
const Ads: React.FC<AdsProps> = ({ business, onUpdate }) => {
    ...
    const handleSave = async () => {
        ...
        let facebookAdId: string | undefined;
        let publishingError: string | undefined;

        // Publish to Facebook if enabled and connected
        if (publishToFacebook && business.facebookAdConfig?.connected) {
            setIsPublishing(true);
            try {
                // 1. Upload image
                const uploadRes = await fetch('/api/facebook/upload-image', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'x-facebook-access-token': business.facebookAdConfig.accessToken || '',
                        'x-facebook-ad-account-id': business.facebookAdConfig.adAccountId || '',
                    },
                    body: JSON.stringify({
                        businessId: business.id,
                        imageBase64: mediaUrl,
                        imageName: `${product?.name || 'ad'}-${Date.now()}.jpg`,
                    }),
                });
                ...
                // 2. Create ad
                const createRes = await fetch('/api/facebook/create-ad', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'x-facebook-access-token': business.facebookAdConfig.accessToken || '',
                    ...
                    body: JSON.stringify({
                        ...
                        whatsappNumber: business.twilioNumber,
                        pageId: business.facebookAdConfig.pageId || '',
                    }),
                });
                ...
                facebookAdId = createData.adId;
                alert(`✅ Ad published to Facebook! Ad ID: ${facebookAdId}`);
            } catch (error: any) {
                console.error('Facebook publishing error:', error);
                publishingError = error.message;
                alert(`⚠️ Ad saved locally, but Facebook publishing failed: ${error.message}`);
            } finally {
        ...
        const newAd: Ad = {
            ...
            platform,
            status: facebookAdId ? 'active' : 'draft',
            headline,
            primaryText,
            mediaUrl,
                ...
                <div className="flex flex-1 gap-8 overflow-hidden">
                    ...
                    <div className="w-1/2 bg-slate-800 rounded-xl border border-slate-700 p-6 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="space-y-6">
                            ...
                            {/* Facebook Publishing Toggle */}
                            {business.facebookAdConfig?.connected && (
                                    ...
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                className="w-4 h-4"
                                            />
                                            <label className="text-sm font-bold text-white">Publish to Facebook Ads Manager</label>
                                        </div>
                                        <span className="text-xs text-green-400">✓ Connected</span>
                                    </div>
                                    <p className="text-xs text-slate-400">
                                        Ad will be created in your Facebook Ad Account: {business.facebookAdConfig.adAccountName || business.facebookAdConfig.adAccountId}
                                    </p>
                                </div>
                            )}
                            ...
                            {!business.facebookAdConfig?.connected && (
                                <div className="bg-yellow-900/20 border border-yellow-700 rounded-xl p-4">
                                    <p className="text-xs text-yellow-400 mb-2">⚠️ Facebook not connected</p>
                                    <p className="text-xs text-slate-400">
                                        Go to Settings → Integrations to connect your Facebook Ad Account.
                                    </p>
                                </div>
                            )}
                            ...
                            <div className="pt-4 border-t border-slate-700 flex justify-end">
                                <button
                                    ...
                                    className="bg-green-600 hover:bg-green-500 text-white px-8 py-3 rounded-lg font-bold shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {isPublishing ? 'Publishing...' : (publishToFacebook && business.facebookAdConfig?.connected ? 'Publish to Facebook' : 'Save Locally')}
                                </button>
                            </div>
                        </div>
                    </div>
