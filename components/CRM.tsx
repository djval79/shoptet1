  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in duration-500 h-[calc(100vh-6rem)] flex flex-col relative">
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
          ...
          {viewMode === 'list' && (
            <div className={`bg-slate-800 rounded-xl border border-slate-700 overflow-hidden shadow-xl flex flex-col transition-all duration-300 flex-1`}>
                <div className="overflow-x-auto flex-1 custom-scrollbar">
                    <table className="w-full text-left">
                    <thead className="bg-slate-900/50 uppercase text-xs font-medium text-slate-400 sticky top-0 z-10">
                        <tr>
                    ...
                    <tbody className="divide-y divide-slate-700 text-sm">
                        ...
                        <tr 
                            key={c.id} 
                            className={`hover:bg-slate-700/30 transition-colors cursor-pointer ${selectedCustomer?.id === c.id ? 'bg-blue-900/10 border-l-2 border-blue-500' : ''}`}
                        ...
                        >
                            ...
                            <div className="flex items-center">
                                <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 flex items-center justify-center text-white font-bold mr-3 text-xs">
                                {c.name.charAt(0)}
                                </div>
                                <div>
                            ...
                            <td className="px-6 py-4">
                            <span className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold border ${getStatusColor(c.status)}`}>
                                {c.status}
                            </span>
                            </td>
                            <td className="px-6 py-4">
                            ...
                            <td className="px-6 py-4">
                                <span className="flex items-center gap-1 text-xs text-slate-300 bg-slate-900/50 px-2 py-1 rounded border border-slate-700/50 w-fit">
                            ...
                            {c.leadScore !== undefined ? (
                                ...
                                <div className="w-16 bg-slate-700 rounded-full h-1.5">
                                    <div 
                                        className={`h-1.5 rounded-full ${c.leadScore > 70 ? 'bg-green-500' : c.leadScore > 40 ? 'bg-yellow-500' : 'bg-red-500'}`} 
                                        style={{width: `${c.leadScore}%`}}
                                    ></div>
                                </div>
                                <span className="text-white font-mono text-xs">{c.leadScore}</span>
                                </div>
                            ) : (
                            ...
                            <button 
                                ...
                                className="text-blue-400 hover:text-blue-300 font-medium text-xs border border-blue-500/30 bg-blue-500/10 px-2 py-1 rounded transition-colors disabled:opacity-50"
                            >
                                {analyzingId === c.id ? '...' : '⚡ Score'}
                            </button>
                            </td>
                        </tr>
                        ))}
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="bg-slate-900/50 p-3 rounded-lg border border-slate-700">
                              ...
                              <p className={`text-xl font-mono font-bold mt-1 ${
                                  (selectedCustomer.leadScore || 0) > 70 ? 'text-green-400' : 'text-yellow-400'
                          ...
                          <div className="relative pl-4 border-l border-slate-700 space-y-6">
                              ...
                              {getTimeline(selectedCustomer).map((event, idx) => (
                                  <div key={idx} className="relative">
                                      <div className={`absolute -left-[21px] top-1 w-3 h-3 rounded-full border-2 border-slate-800 ${
