...
const Connect: React.FC<ConnectProps> = ({ business, onUpdate }) => {
  ...
  const [incomingUrl, setIncomingUrl] = useState(business.sandboxConfig?.incomingUrl || `https://api.twilioflow.ai/v1/webhooks/whatsapp/${business.id}/incoming`);
  const [statusCallbackUrl, setStatusCallbackUrl] = useState(business.sandboxConfig?.statusCallbackUrl || `https://api.twilioflow.ai/v1/callbacks/status/${business.id}`);
  const [fallbackUrl, setFallbackUrl] = useState(business.sandboxConfig?.fallbackUrl || `https://api.twilioflow.ai/v1/callbacks/fallback/${business.id}`);
...
<script src="https://cdn.twilioflow.ai/widget/v1/bundle.js" async></script>
  `.trim();
